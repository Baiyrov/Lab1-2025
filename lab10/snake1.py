...
def insert_or_update_score(name, score, level): 
    conn = psycopg2.connect(dbname='snake', user='postgres', password='Erasyl2007', host='localhost', port='5432') 
    cur = conn.cursor() 
    select_query = "SELECT score FROM snake_game_scores WHERE player_name = %s" 
    cur.execute(select_query, (name,)) 
    row = cur.fetchone() 
    if row: 
        if score > row[0]: 
            update_query = "UPDATE snake_game_scores SET score = %s, level = %s WHERE player_name = %s" 
            cur.execute(update_query, (score, level, name)) 
    else: 
...
def get_high_score(name): 
    conn = psycopg2.connect(dbname='snake', user='postgres', password='Erasyl2007', host='localhost', port='5432') 
    cur = conn.cursor() 
    query = "SELECT score, level FROM snake_game_scores WHERE player_name = %s" 
...
high_score, high_level = get_high_score(player_name)

print(f"Welcome back {player_name}! Your current high score is {high_score} at level {high_level}.")
    ...
    while True: 
        for event in pygame.event.get(): 
            if event.type == pygame.QUIT: 
                insert_or_update_score(player_name, score, level) 
                pygame.quit() 
                sys.exit() 
