...
start_pos = None
last_pos = None  # Для плавного рисования
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == MOUSEMOTION and event.buttons[0]:  # Если зажата левая кнопка мыши
            if current_tool in ["pen", "eraser"] and last_pos:
                draw_color = WHITE if current_tool == "eraser" else current_color
                pygame.draw.line(canvas, draw_color, last_pos, event.pos, brush_size if current_tool == "pen" else eraser_size)
                last_pos = event.pos
        ...
        if event.type == MOUSEBUTTONUP:
            if event.button == 1 and start_pos:  # Если отпустили левую кнопку мыши и есть начальная позиция
                ...
                start_pos = None
                last_pos = None  # Сбрасываем последнюю позицию
